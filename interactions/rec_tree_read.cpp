...
Node* get_trig (char* string, int* pointer, Errors *const error)
{
    ...
    strncpy(op_name, get_str(string + *pointer, op_name), MAX_STR_LEN);  //получается, что указатель не сдвинентся, мы ничего не потеряем, но узнаем, есть ли так хать что-то
    ...
    if (strlen(op_name) == 0)
        get_add_sub (string, pointer, error);
